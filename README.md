...
A labelled hostile ground marker appears on the map.
...
Signal side — mode, account, group:

```bash
curl -s http://127.0.0.1:8080/v1/about                    # "mode": "json-rpc"
curl -s http://127.0.0.1:8080/v1/accounts                 # ["{your_number}"]
curl -s http://127.0.0.1:8080/v1/groups/{your_number}     # id matches GROUP_ID
```

taky side — TLS reachability without involving Signal. Save as `probe.py` in the project
root (`load_dotenv()` looks for `.env` beside the calling file) and run it:
...
load_dotenv()
d = Path("taky-server/ssl")
ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
ctx.load_verify_locations(d / "ca.crt")
ctx.load_cert_chain(d / "server.crt", d / "server.key")
ctx.check_hostname = False
host, port = os.environ["TAK_HOST"], int(os.environ["TAK_PORT"])
with socket.create_connection((host, port), timeout=5) as raw:
    with ctx.wrap_socket(raw) as s:
        print("TLS OK:", s.version())          # TLS OK: TLSv1.3
```

Triggering the bot without touching the phone — a message sent from the linked number
through the API does not reach the bot, because the bot's own signal-cli is the sender and
no sync message comes back to it. `notify_self` forces one:

```bash
curl -s -X POST http://127.0.0.1:8080/v2/send \
  -H 'Content-Type: application/json' -d '{
    "message": "48.567123 39.87897 tank",
    "number": "{your_number}",
    "notify_self": true,
    "recipients": ["{group_id}"]
  }'
